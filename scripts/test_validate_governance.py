...
Every mutable governance state used by these tests is built from the pristine
fixture under ``scripts/fixtures/governance_template/`` rather than from the
live ``.agent/`` directory. The live directory holds the real adopted project
state, so it is only ever exercised as an adopted-mode positive case and as a
template-mode rejection case.
"""
...
TEMPLATE_FIXTURE = ROOT / "scripts/fixtures/governance_template"
GOVERNANCE_FILES = (
    "PROJECT_GOAL.md", "PROJECT_PROFILE.md", "CURRENT.md", "DIRECTIVES.md",
    "OUTCOMES.md", "LEARNINGS.md", "RECORD.md", "REPO_MAP.md",
)
...
def fixture_text(relative: str) -> str:
    return (TEMPLATE_FIXTURE / ".agent" / relative).read_text(encoding="utf-8")


def make_template(root: Path) -> None:
    """Replace the copied live governance state with the pristine fixture."""
    for relative in GOVERNANCE_FILES:
        (root / ".agent" / relative).write_text(fixture_text(relative), encoding="utf-8")


def template_repo(destination: Path) -> None:
...
def make_fresh_adopted(root: Path) -> None:
    make_adopted(root)
    current = fixture_text("CURRENT.md")
    ...
    for relative in ("DIRECTIVES.md", "OUTCOMES.md", "LEARNINGS.md", "RECORD.md"):
        (root / ".agent" / relative).write_text(fixture_text(relative), encoding="utf-8")
...
def main() -> int:
    live = run(ROOT, "ADOPTED")
    if live.returncode != 0:
        raise AssertionError(f"live repository failed adopted validation\n{live.stdout}\n{live.stderr}")

    with tempfile.TemporaryDirectory(prefix="governance-positive-") as directory:
        template = Path(directory) / "template"
        template_repo(template)
        result = run(template, "TEMPLATE")
        if result.returncode != 0:
        ...
        live_copy = Path(directory) / "live-copy"
        copy_repo(live_copy)
        if run(live_copy, "TEMPLATE").returncode == 0:
    ...
    print("- Live repository passed adopted validation")
    print("- Live adopted governance state rejected in template mode")
    print(f"- Template positive fixture plus {len(template_cases)} template rejection cases")
    print(f"- ADOPTED positive fixture plus {len(adopted_cases)} ADOPTED rejection cases")
